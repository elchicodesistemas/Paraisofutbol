...
begin
 ...
 if (p_data->>'birth_date')::date>((now() at time zone 'America/Argentina/Buenos_Aires')::date-interval '18 years')::date then raise exception 'El adulto responsable debe tener al menos 18 años.';end if;
 ...
 birth:=(p_data->>'birth_date')::date;age_year:=extract(year from (now() at time zone 'America/Argentina/Buenos_Aires'))::int-extract(year from birth)::int;cat:=nullif(p_data->>'category_id','')::uuid;
 if cat is null then
